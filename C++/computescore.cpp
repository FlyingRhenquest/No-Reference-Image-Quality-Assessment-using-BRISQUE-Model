...
namespace jd_brisque {
  ...
  /**
   * load_model loads the svm model from the filename. This allocates a svm_model *,
   * which must be destroyed with svm_free_and_destroy_model later. Will
   * return null on error (File not found et al.)
   */

  svm_model *load_model(std::string filename)
  {
    svm_model *retval = nullptr;
